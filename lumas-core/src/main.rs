//! # Lumas Core Process
//!
//! The central intelligence subsystem of the Lumas platform.
...
use lumas_common::input::{HotkeyAction, InputEvent, InputEventType, MouseButton};
use lumas_common::ipc::{Channel, LumiMessage, MessageType, ProcessId};
use lumas_common::logging::{AuditEntry, AuditOutcome, LogLevel};
use lumas_common::performance::FramePacerConfig;
use lumas_common::privacy::PIIAction;
use lumas_common::state_machine::{StateCommand, StateEvent, default_transition_rules};
    ...
    pub fn new(bus: MessageBus) -> Self {
        Self {
            ...
            response_cache: Arc::new(RwLock::new(ResponseCache::new(
                lumas_common::performance::ResponseCacheConfig::default(),
            ))),
...
async fn main() -> anyhow::Result<()> {
    tracing_subscriber::fmt()
        .with_env_filter(
            tracing_subscriber::EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| "lumi_core=info,lumas_ipc=info".into()),
        )
        .init();

    info!("Starting Lumas Core Process...");
    ...
    info!("Lumas Core Process running");
    ...
    info!("Lumas Core Process shut down");
    Ok(())
}
...
impl CoreState {
    ...
    async fn handle_hotkey_action(&self, action: HotkeyAction) {
        match action {
            HotkeyAction::ToggleConversation => {
                self.state_machine
                    ...
                    .handle_event(StateEvent::UserInput {
                        source: lumas_common::state_machine::InputSource::Keyboard,
                        content: String::new(),
                    });
            }
            ...
            HotkeyAction::ToggleFocusMode => {
                ...
                let desktop = self.desktop.write().await;
    ...
    async fn handle_plugin_capability(&self, msg: LumiMessage) {
        ...
        if let Some(_plugin_name) = msg.payload.get("plugin").and_then(|p| p.as_str()) {
            ...
            match event_type {
                "focus_detected" => {
                    ...
                    self.emotion
                        .write()
                        .await
                        .transition_to(lumas_common::emotion::Emotion::Calm, 0.3);
                }
                "focus_ended" => {
                    self.state_machine
                        .write()
                ...
                "user_active" => {
                    ...
                    self.desktop
                        .write()
                        .await
                        .register_input(lumas_common::desktop::InputType::Keyboard);
                }
                "user_idle" => {
        ...
        if msg.msg_type == MessageType::Request {
            ...
            if let Some(key) = msg.payload.get("key").and_then(|k| k.as_str()) {
                ...
                if is_sensitive {
                    ...
                    match action {
                        lumas_common::security::ApprovalAction::Deny => {
                            let response =
...
mod tests {
    ...
    fn test_security_approval_gate() {
        ...
        match action {
            lumas_common::security::ApprovalAction::AutoApprove => {}
