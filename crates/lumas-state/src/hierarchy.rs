...
use crate::state::{MachineState, StateContext, StateTimeout};
...
#[async_trait]
impl MachineState for CompositeState {
    fn id(&self) -> StateId {
        self.id
    }
    ...
    ) -> Pin<Box<dyn std::future::Future<Output = crate::error::StateResult<()>> + Send + 'a>> {
...
pub fn find_ancestor_composites<'a>(
    state_id: StateId,
    hierarchy: &std::collections::HashMap<StateId, StateId>,
    composites: &'a std::collections::HashMap<StateId, &'a CompositeState>,
) -> Vec<&'a CompositeState> {
