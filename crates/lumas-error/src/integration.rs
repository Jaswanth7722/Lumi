...
//!
//! Provides zero-boilerplate integration for every Lumas subsystem.
//! Macros auto-generate `From<SubsystemError> for LumasError` implementations
//! and register subsystems in the RecoveryEngine.
//!
//! # Macros
//!
//! - `register_subsystem!` — one-liner subsystem registration
//! - `impl_from_error!` — generates `From<E> for LumasError` for any error type
//!
//! # ErrorBridge
//!
//! `ErrorBridge<E>` wraps existing non-LumasError types so they can report
...
/// A wrapper that lets existing code with non-LumasError errors report
/// through the framework without a full migration.
///
/// # Example
///
...
///     .with_category(ErrorCategory::Filesystem { path: None, operation: lumas_error::category::FilesystemOp::Read })
///     .with_severity(Severity::Recoverable);
/// let lumi_err: LumasError = bridge.into();
/// ```
#[derive(Debug)]
...
impl<E: std::error::Error + Send + Sync + 'static> From<ErrorBridge<E>> for LumasError {
    fn from(bridge: ErrorBridge<E>) -> Self {
        let msg = bridge.inner.to_string();
        LumasError::new(bridge.code, bridge.category.clone(), &msg)
            .with_severity(bridge.severity)
            .with_diagnostic_message(format!("{}: {}", bridge.category, msg))
    }
}

/// Macro to generate `From<E> for LumasError` implementations.
///
/// # Example
///
/// ```ignore
...
macro_rules! impl_from_error {
    (error_type: $ty:ty, code: $code:expr, category: $cat:expr, severity: $sev:expr $(,)?) => {
        impl From<$ty> for $crate::error::LumasError {
            fn from(err: $ty) -> Self {
                $crate::error::LumasError::new($code, $cat, err.to_string()).with_severity($sev)
            }
        }
    };
}

/// Generates `From<$wrapper> for LumasError` where `$wrapper` is a newtype
...
macro_rules! wrap_error {
    ($wrapper:ty, $code:expr, $cat:expr, $sev:expr $(,)?) => {
        impl From<$wrapper> for $crate::error::LumasError {
...
mod tests {
    ...
    fn test_error_bridge_defaults() {
        let io_err = std::io::Error::new(std::io::ErrorKind::NotFound, "file not found");
        let bridge: LumasError = ErrorBridge::new(io_err).into();
