...
//! - `LumasErrorBuilder` — fluent builder for test error construction
...
/// Fluent builder for constructing LumasErrors in tests.
#[derive(Debug)]
pub struct LumasErrorBuilder {
    code: ErrorCode,
    category: ErrorCategory,
    message: String,
    severity: Severity,
    source: Option<LumasError>,
}

impl LumasErrorBuilder {
    ...
    pub fn with_source(mut self, source: LumasError) -> Self {
        self.source = Some(source);
        self
    }

    /// Build the LumasError.
    pub fn build(self) -> LumasError {
        let mut err =
            LumasError::new(self.code, self.category, self.message).with_severity(self.severity);
...
impl Default for LumasErrorBuilder {
    fn default() -> Self {
        Self::new()
    }
}
...
impl MockRecoveryEngine {
    ...
    pub fn recover(&mut self, error: &LumasError, strategy: &RecoveryStrategy) -> RecoveryOutcome {
...
mod tests {
    ...
    fn test_error_builder() {
        let error = LumasErrorBuilder::new()
