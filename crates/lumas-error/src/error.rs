//! # LumasError — Core Error Type
//!
//! The root error type for the entire Lumas platform.
...
/// The root error type for the Lumas platform.
///
/// LumasError is a structured error type that:
...
impl LumasError {
    /// Create a new LumasError with the given code and category.
...
impl fmt::Display for LumasError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "{}", self.user_message)
    }
}

impl std::error::Error for LumasError {
...
// So we don't need a manual From<LumasError> for Box<dyn ...> impl.

// Manual Clone impl: source field (Box<dyn Error>) doesn't implement Clone
impl Clone for LumasError {
    fn clone(&self) -> Self {
        Self {
...
/// Convenience result type for LumasError.
pub type LumiResult<T> = Result<T, LumasError>;

impl<T> LumiResultExt for Result<T, LumasError> {
    fn context(self, msg: impl Into<String>) -> Self {
        self.map_err(|e| {
            LumasError::new(e.code(), e.category().clone(), msg)
...
mod tests {
    ...
    #[test]
    fn test_result_ext() {
        let result: LumiResult<()> = Err(LumasError::new(
            ErrorCode::CONFIG_FILE_NOT_FOUND,
            ErrorCategory::Configuration { field: None },
            "original",
        ));
