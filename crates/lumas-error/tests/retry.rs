...
async fn test_retry_fatal_does_not_retry() {
    ...
    let result = retry(&policy, || async {
        attempts.fetch_add(1, Ordering::SeqCst);
        Err::<(), LumasError>(
            LumasError::new(ErrorCode::RUNTIME_INTERNAL, ErrorCategory::Runtime, "fatal")
...
async fn test_retry_condition_recoverable_only() {
    ...
    let fatal_err = LumasError::new(ErrorCode::RUNTIME_INTERNAL, ErrorCategory::Runtime, "fatal")
        .with_severity(Severity::Fatal);
