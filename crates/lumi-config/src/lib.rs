//! # Lumi Configuration System
//!
//! Complete, production-ready configuration system for the Lumi platform.
...
//! in the Lumi platform. It replaces and extends the previous inline config
//! structs that were in lumi-common and lumi-runtime.
//!
//! Existing config-related items found during workspace audit:
//! - lumi-config/src/lib.rs (old): Had LumiConfig with flat config structs
//! - lumi-runtime/src/config.rs: Had ConfigLoader wrapping lumi-config
//! - lumi-runtime/src/event.rs: Had ConfigLoaded, ConfigReloaded events
//!
//! Design decisions:
//! - lumi-config does NOT depend on lumi-runtime to avoid circular deps
//! - Config events are standalone types; a ConfigEventPublisher trait
//!   bridges to lumi-runtime's event bus
