...
use lumi_ipc::wire::codec::{encode_message, decode_frame, WireCodec, WireCodecConfig};
use lumi_ipc::wire::frame::{LumiFramer, RawFrame};
use lumi_ipc::wire::metrics::WireMetrics;

fn bench_encode_small(c: &mut Criterion) {
    let codec = WireCodec::new(WireCodecConfig::default());
    let msg = lumi_ipc::message::LumiMessage::new(
        lumi_ipc::message::MessageKind::Data,
        lumi_ipc::message::MessagePayload::Empty,
    );
...
fn bench_header_parse(c: &mut Criterion) {
    ...
    c.bench_function("header_parse_valid", |b| {
        b.iter(|| {
            let h = lumi_ipc::wire::header::Header::parse(black_box(&header_bytes));
            black_box(h)
        })
    });
}
