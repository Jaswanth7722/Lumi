...
use lumi_error::crash::CrashReport;
use lumi_error::error::LumiError;
use lumi_error::error_code::ErrorCode;
use lumi_error::prelude::*;
use lumi_error::report::{ErrorReport, ErrorReportBatch, ReportFormat};
...
async fn test_report_batch_serialization() {
    let err1 = LumiError::new(
        ErrorCode::AI_INFERENCE_FAILED,
        ErrorCategory::AiCore { provider: None },
        "error 1",
    );
    let err2 = LumiError::new(
        ErrorCode::CONFIG_FILE_NOT_FOUND,
        ErrorCategory::Configuration { field: None },
        "error 2",
    );
...
async fn test_crash_report_serialization() {
    let report = CrashReport::new(lumi_error::crash::CrashType::FatalError);
...
async fn test_crash_report_with_metadata_serialization() {
    let report = CrashReport::new(lumi_error::crash::CrashType::Panic {
...
async fn test_metrics_snapshot_serialization() {
    let metrics = lumi_error::metrics::ErrorMetrics::new();
...
async fn test_diagnostic_report_serialization() {
    let history = lumi_error::diagnostics::ErrorHistory::new(100);
    let err = LumiError::new(
        ErrorCode::AI_INFERENCE_FAILED,
        ErrorCategory::AiCore { provider: None },
        "test",
    ...
    let report = lumi_error::diagnostics::generate_diagnostic_report(&history);
    let json = serde_json::to_string(&report).unwrap();
    let parsed: serde_json::Value = serde_json::from_str(&json).unwrap();
    assert_eq!(parsed["total_errors"], 1);
}
