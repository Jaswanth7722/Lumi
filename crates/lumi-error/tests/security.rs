...
use lumi_error::report::{ErrorReport, ReportFormat};
...
async fn test_error_code_format_no_secrets() {
    let err = LumiError::new(
        ...
        ErrorCategory::Security {
            violation: lumi_error::category::SecurityViolation::Authentication,
        },
        "Access denied",
    );
...
async fn test_user_safe_output_no_stack_traces() {
    ...
    let formatter = lumi_error::formatter::ErrorFormatter::compact_user();
...
async fn test_diagnostic_output_has_location() {
    ...
    let formatter = lumi_error::formatter::ErrorFormatter::detailed_diagnostic();
    let output = formatter.format(&err);
