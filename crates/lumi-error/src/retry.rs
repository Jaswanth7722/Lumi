...
#[derive(Clone)]
pub struct RetryCondition(Arc<dyn Fn(&LumiError) -> bool + Send + Sync>);

impl RetryCondition {
    /// Create a new retry condition.
    pub fn new<F>(f: F) -> Self
    where
        F: Fn(&LumiError) -> bool + Send + Sync + 'static,
    {
        Self(Arc::new(f))
    }

    /// Check if the error should be retried.
    pub fn should_retry(&self, error: &LumiError) -> bool {
        (self.0)(error)
    }
...
pub struct RetryAttempt {
    ...
    pub last_error: Option<Box<LumiError>>,
...
pub async fn retry<F, Fut, T>(policy: &RetryPolicy, op: F) -> Result<T, LumiError>
where
    F: Fn() -> Fut + Send,
    Fut: std::future::Future<Output = Result<T, LumiError>> + Send,
{
...
mod tests {
    ...
    fn test_retry_condition() {
        ...
        // Can't easily test with a real LumiError in a simple test
    }
}
