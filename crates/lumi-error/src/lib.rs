//! # lumi-error — Centralized Error Handling for the Lumi Platform
//!
//! ## Architecture Overview
//!
//! Every subsystem in the Lumi workspace communicates failures exclusively
//! through `lumi-error`. This crate provides:
//!
//! - **[`LumiError`](error::LumiError)** — Root sum type over all subsystem error categories
...
// |-------|------|---------------------|--------|
// | lumi-runtime | src/error.rs | RuntimeError enum (8 variants), RuntimeResult<T> alias | REPLACE → LumiError |
// | lumi-runtime | src/event.rs | Event trait (Send+Sync+Clone+Debug+'static) | REUSE |
// | lumi-config | src/error.rs | ConfigError enum (12 variants) with is_recoverable/suggested_action | REPLACE → LumiError |
// | lumi-config | src/validator.rs | ValidationError, Validate trait | REPLACE → LumiError |
// | lumi-logging | src/error.rs | LogError enum (13 variants) | REPLACE → LumiError |
// | lumi-common | src/security.rs | SecretDescriptor, Error enum | REPLACE → LumiError |
// | lumi-common | src/ipc.rs | various error types | REPLACE → LumiError |
// | lumi-core | src/main.rs | uses tracing for errors | REUSE via ErrorFormatter |
//
// ## Migration Strategy
// 1. Phase 1: Define LumiError as the unified error type (this crate)
...
pub use error::{
    DiagnosticMessage, LumiError, LumiResult, LumiResultExt, RecoveryHint, UserFacingMessage,
};
...
pub mod prelude {
    ...
    pub use crate::error::{LumiError, LumiResult, LumiResultExt};
