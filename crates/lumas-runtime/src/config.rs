...
//! 3. **Environment Overrides** — `LUMAS_{SECTION}_{KEY}` env vars
...
use lumas_config::LumiConfig;
use lumas_config::{CloudProvider, InferenceMode};
...
impl ConfigLoader {
    ...
    ///
    /// Convention: `LUMAS_{SECTION}_{KEY}` (e.g., `LUMAS_AI_INFERENCE_MODE`).
    fn apply_env_overrides(&self, config: &mut LumiConfig) -> Result<(), Vec<ConfigError>> {
        let mut errors = Vec::new();

        for (key, value) in std::env::vars() {
            if !key.starts_with("LUMAS_") {
                continue;
            }

            let config_key = &key[5..]; // Strip "LUMAS_"
    ...
    fn merge_configs(base: &mut LumiConfig, overlay: LumiConfig) {
        ...
        // Character
        if overlay.character.name != "Lumas" {
