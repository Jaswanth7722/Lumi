...
//! - lumi-runtime/src/event.rs: Event trait (`pub trait Event: Send + Sync + Clone + fmt::Debug + 'static`)
//!   with existing ConfigLoaded, ConfigReloaded, LifecycleTransitioned event types
//! - lumi-runtime/src/metrics.rs: MetricsRegistry with Counter, Gauge, Histogram, Timer
//! - lumi-config/src/secret.rs: Secret<T> newtype with redacted Debug/Display/Serialize
//! - lumi-config/src/schema/logging.rs: LoggingConfig struct (with level, format, file_logging, etc.)
