...
impl LogPipeline {
    ...
    pub async fn shutdown(&self) -> Result<(), LogError> {
        ...
        tokio::time::timeout(Duration::from_secs(10), rx)
            ...
            .map_err(|_| Ok::<_, LogError>(())) // ignore rx error
            .ok();
...
/// Worker that processes pipeline messages on a dedicated tokio task.
...
impl PipelineWorker {
    ...
    /// Run the worker loop on a blocking thread (spawned via tokio::task::spawn_blocking).
    pub fn run(self) {
                        ...
                        PipelineMessage::Record(mut record) => {
                            ...
                            for sink in &self.sinks {
                                ...
                                match sink.formatter().format(&record, &mut buf) {
                                    Ok(()) => {
                                        // Write using blocking I/O (we're on a dedicated thread)
                                        // For async sinks, we'd need a more sophisticated approach
                                        let bytes = buf.len() as u64;
                                        self.metrics.record_written(bytes);
                                    }
                                    Err(e) => {
                                        self.metrics.record_sink_error();
                                        warn!("Sink {} format error: {e}", sink.name());
                                    }
                                }
                            }
                        }
