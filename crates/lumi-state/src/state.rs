...
pub enum TimeoutAction {
    ...
    Transition(crate::event::EventId),
...
impl MachineState for LeafState {
    ...
    async fn on_entry<'a>(&'a self, _ctx: &'a mut StateContext) -> StateResult<()> {
        Ok(())
    }
    async fn on_exit<'a>(&'a self, _ctx: &'a mut StateContext) -> StateResult<()> {
        Ok(())
    }
}
