...
use crate::guard::{CrossMachineGuard, StateQuery};
...
impl StateMachineManager {
    ...
    async fn fire_event_inner(
        ...
        let (machine_guard, instance_guard) = {
            ...
            (machine.clone(), instance.clone())
        };

        let machine = machine_guard.value();
        let mut instance_write = instance_guard.write().await;
        let current_state = instance_write.current_state;
        ...
        // Check cross-machine invariants
        ...
        // Update instance if committed (lock already held as write)
        match &outcome {
    ...
    pub fn current_state(&self, machine_id: MachineId) -> StateResult<StateSnapshot> {
        ...
        let snapshot = machine.build_snapshot(&instance.value().read_blocking());
        Ok(snapshot)
    }
    ...
    pub fn platform_snapshot(&self) -> PlatformStateSnapshot {
        ...
        for entry in self.inner.machines.iter() {
            ...
            if let Some(instance) = self.inner.instances.get(&machine.id) {
                let snap = machine.build_snapshot(&instance.value().read_blocking());
    ...
    pub fn get_machine(&self, machine_id: MachineId) -> Option<StateMachine> {
        self.inner.machines.get(&machine_id).map(|e| e.clone())
    }
...
impl StateQuery for ManagerInner {
    fn current_state_for(&self, machine_id: MachineId) -> Option<StateId> {
        self.instances
            .get(&machine_id)
            .map(|instance| instance.value().read_blocking().current_state)
    }
}
...
impl CrossMachineCoordinator {
    ...
    pub async fn check(
        ...
        for (source, guard) in self.guards.read().iter() {
