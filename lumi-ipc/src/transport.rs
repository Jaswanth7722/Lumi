...
//!
//! Each Lumi process creates a listener that accepts connections from peers
//! and also initiates outbound connections to other processes.

use anyhow::{Result, anyhow, bail};
use lumi_common::ipc::ProcessId;
...
impl TransportListener {
    ...
    pub async fn bind(process_id: &ProcessId, runtime_dir: &Path) -> Result<Self> {
        let socket_path = Self::socket_path(process_id, runtime_dir);
    ...
    pub async fn accept(&self) -> Result<Box<dyn IoStream>> {
        match self {
            ...
            #[cfg(windows)]
            TransportListener::NamedPipe(server) => {
